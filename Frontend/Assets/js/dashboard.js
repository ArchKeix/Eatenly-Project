...
async function loadUserData(idUser) {
  ...
  try {
    const response = await fetch(`https://eatenly-project-production-4524.up.railway.app/personalize/${idUser}`);
    if (response.ok) {
...
async function loadRiwayatFromBackend() {
  ...
  try {
    const response = await fetch('https://eatenly-project-production-4524.up.railway.app/ai/history', {
      method: 'GET',
      headers: {
...
async function handleFileSelect(input) {
  if (input.files && input.files[0]) {
    ...
    try {
      const response = await fetch('https://eatenly-project-production-4524.up.railway.app/ai/', {
        method: 'POST',
